...
var Deferred = function () {
  ...
  function resolve(data) {
    ...
    while(tmpEventItem = thenEventsHandler.shift()) {
      ...
      if (tmpResult instanceof Promise || (tmpResult && typeof tmpResult.then === 'function')) {
        // 如果 then 的成功回调返回的也是 promise , 则将当前 promise 的事件处理函数传递给该该 promise
        // 继续执行新 promise
        promise = tmpResult;
        var innerEventItem;
        while(innerEventItem = thenEventsHandler.shift()) {
          promise.addEvent(innerEventItem);
        }
        break;
      }
    }
  ...
  // 更新当前 promise 的成功失败回调栈
  Promise.prototype.addEvent = function (item) {
    thenEventsHandler.push(item);
  }

  Promise.prototype.getEvent = function () {
    return thenEventsHandler;
  }
